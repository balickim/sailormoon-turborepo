...
import { spawn } from 'child_process'; // Import spawn to run the Go server
import icon from '../../resources/icon.png?asset';

let goServerProcess; // Variable to hold the Go server process
...
function createWindow(): void {
  ...
  // Ensure Go server stops when the window is closed
  mainWindow.on('close', () => {
    stopGoServer();
  });
}
...
function startGoServer() {
  const serverPath = join(__dirname, '../../backend'); // Path to your Go server executable
